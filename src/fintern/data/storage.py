...
IGNORED_DATASET_FILES = {"_SUCCESS", "_metadata", "_common_metadata"}
...
def detect_path_format(path: str | Path) -> str:
...
def detect_file_type(path_type: str, path: str | Path) -> str:
    path = normalize_path(path)
    ...
    for start_index in range(len(suffixes)):
        ...
        if candidate in SUPPORTED_FILE_TYPES:
            return SUPPORTED_FILE_TYPES[candidate]
...
def iter_supported_files(path: Path) -> list[Path]:
    ...
    for item in sorted(path.rglob("*")):
        ...
        if item.name in IGNORED_DATASET_FILES:
            continue

        if detect_file_type("file", item) != "unknown":
            files.append(item)
...
def describe_tabular_path(path: str | Path) -> PathDescription:
    normalized_path = normalize_path(path)
    path_type = detect_path_format(normalized_path)

    if path_type == "file":
        file_type = detect_file_type(path_type, normalized_path)
    ...
    leaf_files = tuple(iter_supported_files(normalized_path))

    if not leaf_files:
        raise ValueError(f"No supported files found under folder: {normalized_path}")

    file_types = tuple(
        sorted({detect_file_type("file", file_path) for file_path in leaf_files})
    )
...
def coerce_to_dataframe(data: Any, source: Path) -> pd.DataFrame:
...
def _read_json_file(path: Path) -> pd.DataFrame:
    for kwargs in ({"lines": True}, {}):
        try:
            return coerce_to_dataframe(pd.read_json(path, **kwargs), path)
        except ValueError:
            continue
    ...
    return coerce_to_dataframe(payload, path)
...
def _read_hdf5_file(path: Path) -> pd.DataFrame:
    try:
        return coerce_to_dataframe(pd.read_hdf(path), path)
        ...
        for key in keys:
            frame = coerce_to_dataframe(pd.read_hdf(path, key=key), path)
...
def read_single_table(path: Path) -> pd.DataFrame:
    ...
    if file_type == "pickle":
        return coerce_to_dataframe(pd.read_pickle(path), path)
...
def load_tabular_dataset(path: str | Path) -> pd.DataFrame | dict[str, pd.DataFrame]:
    description = describe_tabular_path(path)

    if description.path_type == "file":
        return read_single_table(description.path)
    ...
    for file_path in description.leaf_files:
        ...
        loaded_files[relative_path] = read_single_table(file_path)
...
def save_to_csv(
    ...
    output_path = normalize_output_path(path, "csv")
    ...
    partitioned_data = prepare_partitioned_data(data, partition_cols)
...
def save_to_parquet(
    ...
    output_path = normalize_output_path(path, "parquet")
    ...
    if not partition_cols:
        data.to_parquet(output_path / "data.parquet", index=False)
        return output_path
